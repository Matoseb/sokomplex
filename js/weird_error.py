...
def moveFrames(obj, offsets):
    ...
    for t in tracks:
        ...
        if   DescID_Equals(tid, posx):
            ...
            offsetKeys(obj, t, offsets.x, None)
        elif DescID_Equals(tid, posy):

            offsetKeys(obj, t, offsets.y, None)
        elif DescID_Equals(tid, posz):
            
            offsetKeys(obj, t, offsets.z, None)
...
def getBlockType(obj, chunkDims, boxes, levels):
    ...
    if "wall" in name:
        ...
        #levels[level]['s'] = 0  #is hidden 1, not hidden 0  levels[level].get('s') or obj[c4d.ID_USERDATA,3]
        
        nextLevelBehaviour(obj, chunkDims, level, levels)
...
def nextLevelBehaviour(obj, chunkDims, currLevel, levels):
    
    pointer = obj[c4d.ID_USERDATA, 2] 
    
    if(pointer):
        layer = pointer.GetLayerObject(doc)
        nextLevel = layer.GetName()
        
        if(nextLevel == str(currLevel)):
            levels[currLevel]['s'] = 'b'
        elif(nextLevel is not None):
            levels[currLevel]['s'] = int(nextLevel)
        else:
            print 'fuck'
...
def getButtons(obj, chunkDims):
    ...
    for i in range(table.GetObjectCount()):
        ...
        if button:
            if "goal" in button.GetName():
                
                p = button.GetLayerObject(doc)
                if(p):
                    buttons.append('goal_'+p.GetName()) 
